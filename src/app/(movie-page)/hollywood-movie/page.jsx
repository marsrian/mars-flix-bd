...
const HollywoodMoviePage = async ({searchParams}) => {
  ...
  const allHollywoodMovie = sortedHollywoodMovies.slice(start, end);

  return (
    <div className="mb-4">
      <h1 className="border-l-4 border-l-red-600 pl-2 text-xl md:text-3xl font-semibold text-blue-gray-800 dark:text-white">
        Hollywood Movie: {hollywoodMovies?.length}
      </h1>
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-4">
        {allHollywoodMovie?.map((hollywoodMovie) => (
          <Link
      ...
      <PaginationControls
        hasNextPage={end < sortedHollywoodMovies.length}
        hasPrevPage={start > 0}
        movieCategory={"hollywood-movie"}
        currentPage={Number(page)}
        totalPages={Math.ceil(sortedHollywoodMovies.length / Number(per_page))}
      />
    </div>
  );
};

export default HollywoodMoviePage;