...
const MovieCategoryPage = () => {
  ...
  function fetchCategories() {
    fetch("/api/movie-categories").then((res) => {
  ...
  async function handleFormSubmit(ev) {
    ...
    const creationPromise = new Promise(async (resolve, reject) => {
      ...
      const res = await fetch("/api/movie-categories", {
  ...
  async function handleDeleteCategory(_id) {
    const deletionPromise = new Promise(async (resolve, reject) => {
      const res = await fetch(`/api/movie-categories?_id=${_id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        resolve();
  ...
  if (profileLoading) {
    return <Loading loadingInfo={`Loading movie Category Info...`} />;
  }
  ...
  return (
    <div className="w-full md:w-[800px] mx-auto md:px-0 mt-6 px-2">
      <DashboardTabs isAdmin={true} />
      <form onSubmit={handleFormSubmit} className={`flex flex-col gap-2 mt-8`}>
        <label className="">
      ...
      <div className={` mt-6`}>
        <h3 className="text-gray-500">Existing Movie Category:</h3>
        <div className="mt-1">
          {categories.length > 0 &&
